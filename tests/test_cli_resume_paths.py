...
def test_temp_part_base_name_is_unique_for_chapter_override() -> None:
    left = build_temp_part_base_name(part_index=2, final_stem_override="04-Section Alpha")
    right = build_temp_part_base_name(part_index=2, final_stem_override="08-Section Beta")
    assert left != right
...
def test_resume_temp_paths_do_not_collapse_to_global_part_name() -> None:
    ...
    base_name = build_temp_part_base_name(part_index=2, final_stem_override="04-Section Alpha")
...
def test_multipart_final_name_keeps_chapter_first_order() -> None:
    ...
    wav_path, mp3_path = compute_part_output_paths(
        ...
        part_index=2,
        multi_part=False,
        base_name="04-Section Alpha",
        group_name=None,
        final_stem_override="04-Section Alpha",
    )
    assert wav_path.name == "04-02 - Section Alpha.wav"
    assert mp3_path.name == "04-02 - Section Alpha.mp3"
...
def test_first_part_gets_01_when_forced_for_known_multipart() -> None:
    ...
    assert wav_path.name == "04-01 - Section Alpha.wav"
    assert mp3_path.name == "04-01 - Section Alpha.mp3"
